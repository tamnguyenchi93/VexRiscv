...
object Briey_SOTB_Config{
  def default = {
    val config = Briey_SOTB_Config(
      bootRomSize = 8 kB,
      onChipRamSize = 64 kB,
...
class Briey_SOTB_Aug2019(config: Briey_SOTB_Config) extends Component{
  import config._
  val debug = true
...
object Briey_SOTB_Aug2019{
  def main(args: Array[String]) {
    val config = SpinalConfig()
    config.generateVerilog({
      val toplevel = new Briey_SOTB_Aug2019(Briey_SOTB_Config.default)
      toplevel
    })
  }
}
