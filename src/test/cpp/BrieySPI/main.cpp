#include "VBrieySPI.h"
#include "VBrieySPI_BrieySPI.h"
//#include "VBriey_Axi4VgaCtrl.h"
//#include "VBriey_VgaCtrl.h"
#ifdef REF
#include "VBrieySPI_RiscvCore.h"
#endif
#include "verilated.h"
#include <stdio.h>
#include <iostream>
...
public:
	VBrieySPI_VexRiscv *cpu;
	ofstream instructionTraces;
	ofstream regTraces;

	VexRiscvTracer(VBrieySPI_VexRiscv *cpu){
		this->cpu = cpu;
...
public:
	VBrieySPI* top;
	Vga(VBrieySPI* top,int width, int height) : Display(width, height){
		this->top = top;
	}
...
class BrieyWorkspace : public Workspace<VBrieySPI>{
public:
	BrieyWorkspace() : Workspace("Briey"){
		...
		axiClk->add(new VexRiscvTracer(top->BrieySPI->axi_core_cpu));
